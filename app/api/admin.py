...
# These five are thin wrappers over app/playback_control.py. The bodies moved
# there so the Sendspin backend can reuse them without app/output importing
# app/api — one implementation, so a speaker's Next and the web UI's Next
# cannot drift.
@router.post("/playback/pause")
async def playback_pause():
    return await playback_control.playback_pause()

@router.post("/playback/resume")
async def playback_resume():
    return await playback_control.playback_resume()
...
# ── Sendspin pairing + zoning — EXPERIMENTAL ──────────────────────────────────
# Admin-gated by the router-level require_admin. No secret is ever served from
# these routes: the operator supplies the speaker's code, jukeplox never displays
# one. Unpair is the revocation control — pairing is the only authority boundary
# a Sendspin speaker has.
...
class SendspinPairRequest(BaseModel):
    method: str
    code: str
    client_id: str = ""


class ZoneDelayRequest(BaseModel):
    delay_ms: int = 0


class SendspinConnectRequest(BaseModel):
    url: str


@router.get("/output/sendspin/speakers")
async def sendspin_speakers():
    """Everything the pairing screen needs: what is visible on the network, what
    is currently trusted, and how pairing actually works.

    The method descriptors are part of the response so an API caller learns the
    contract here rather than by provoking 400s."""
    b = _sendspin_backend()
    return {
        "discovered": await b.discovered_speakers(),
        "paired": await b.paired_speakers(),
        "methods": [
            {"id": sendspin_adapter.PAIR_STATIC_PIN,
             "label": "Fixed code on the device",
             "code_format": "exactly 8 digits",
             "requires_client_id": True},
            {"id": sendspin_adapter.PAIR_DYNAMIC_PIN,
             "label": "Code shown on the device",
             "code_format": "the digits the device displays",
             "requires_client_id": True},
            {"id": sendspin_adapter.PAIR_TOKEN,
             "label": "Pairing token supplied with the device",
             "code_format": "the token string; it names its own speaker",
             "requires_client_id": False},
        ],
    }


@router.post("/output/sendspin/connect")
async def sendspin_connect(body: SendspinConnectRequest):
    """Dial out to a speaker mDNS never surfaced (different subnet, flaky
    multicast) — the escape hatch for when the discovered list is empty, which
    is exactly when the normal pairing path is unusable."""
    try:
        await _sendspin_backend().connect_speaker(body.url)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return {"ok": True}


@router.post("/output/sendspin/client/{client_id}/cancel-pairing")
async def sendspin_cancel_pairing(client_id: str):
    """Abandon a pairing attempt without waiting out its window — for the
    operator who picked the wrong speaker."""
    await _sendspin_backend().cancel_pairing(client_id)
    return {"ok": True}


@router.post("/output/sendspin/pair")
async def sendspin_pair(body: SendspinPairRequest):
    """Pair with a speaker using the code read off that speaker. jukeplox never
    displays a code for you to type into the speaker — this protocol has no such
    flow, and the previous endpoint had the direction backwards."""
    try:
        client_id = await _sendspin_backend().pair_speaker(
            method=body.method, code=body.code, client_id=body.client_id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return {"ok": True, "client_id": client_id}


@router.post("/output/sendspin/client/{client_id}/unpair")
async def sendspin_unpair(client_id: str):
    """Revoke a speaker. Because pairing is the only authority boundary, this is
    the security control, not a tidy-up."""
    await _sendspin_backend().unpair_speaker(client_id)
    return {"ok": True}
...
@router.post("/output/sendspin/client/{client_id}/delay")
async def sendspin_client_delay(client_id: str, body: ZoneDelayRequest):
    """Per-room latency trim — Sendspin-local for now (see the backend note)."""
    await _sendspin_backend().set_client_delay(client_id, body.delay_ms)
    return {"ok": True}
...
async def playback_skip():
    return await playback_control.playback_skip()

@router.post("/playback/previous")
async def playback_previous():
    return await playback_control.playback_previous()
...
async def playback_volume(body: VolumeRequest):
    return await playback_control.playback_volume(body.level)
