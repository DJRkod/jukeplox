...
async def scan_and_replace(registry, enabled_section_keys: set | None = None) -> bool:
    ...
    if enabled_section_keys is not None and not enabled_section_keys:
        # Sources exist but nothing is effectively enabled (all libraries disabled or
        # all sources vetoed). Clear the catalog rather than fall through to the
        # don't-wipe guard below — that guard exists for TRANSIENT crawl failure, not
        # intentional emptiness, so without this an all-off state would keep serving a
        # stale catalog (Libraries-panel U2).
        await store.replace_catalog([], [], [], [])
        _log.info("Catalog scan: nothing effectively enabled — catalog cleared")
        return True
