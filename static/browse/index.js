...
(function () {
  ...
  function _setupBroadTier(data) {
    ...
    const b = {
      ...
      shown, sentinel, headerDone: false, observer: null,
    };
    b.observer = new IntersectionObserver((entries) => {
      if (entries.some(e => e.isIntersecting)) _loadBroadPage();
    }, { root: _findScrollAncestor(el) || null, rootMargin: '200px' });
  ...
  async function _loadBroadPage() {
    ...
    try {
      ...
      // Leave the tier idle on failure; a later scroll re-enters the sentinel
      // and retries this same page.
    } finally {
      if (_broad === b) {
        b.loading = false;
        ...
        // out) or the tier is done.
        if (!b.done && b.observer) {
