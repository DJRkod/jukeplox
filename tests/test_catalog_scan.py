...
async def test_scan_and_replace_empty_enabled_keys_clears_catalog(db):
    """All-off (every library disabled or every source vetoed) yields an EMPTY
    enabled set. Unlike a transient crawl failure, this is INTENTIONAL emptiness, so
    the catalog is cleared rather than left stale — the P0 the doc review caught:
    without this, an all-vetoed install keeps serving the stale table forever
    (Libraries-panel U2)."""
    await store.replace_catalog([], [_album_row()], [_track_row()], [])
    assert len(await store.get_all_tracks()) == 1
    src = _FakeSource(
        "m1", libs=[Library(key="m1:1", title="M", type="artist", server_name="P")],
        artists=[_artist("m1:ar")], albums=[_album("m1:al")], tracks=[_track("m1:t1", "m1:al")],
    )
    ok = await scan.scan_and_replace(_FakeRegistry([src]), set())
    assert ok is True
    assert await store.get_all_tracks() == []  # cleared, not stale


async def test_scan_and_replace_nonempty_keys_all_failed_does_not_wipe(db):
    """A real enabled set whose crawl transiently fails must NOT wipe — only an
    EMPTY set is intentional emptiness. Guards the empty-vs-transient distinction."""
    await store.replace_catalog([], [_album_row()], [_track_row()], [])
    bad = _FakeSource("m1", libs=[], fail=True)
    ok = await scan.scan_and_replace(_FakeRegistry([bad]), {"m1:1"})
    assert ok is False
    assert len(await store.get_all_tracks()) == 1  # transient failure keeps prior state
